...
Auth PASS 2 now provides the approved Crew-invitation post-auth continuation and generic HMAC-evidence rate limiter. Website PASS 3 consumes those interfaces without modifying Auth-owned files. The raw invitation bearer token exists only at the initial Website landing seam; after validation Website issues the Auth continuation and redirects immediately, so ordinary login/Google/post-auth navigation and the acceptance form carry no raw invitation token. Final acceptance revalidates the exact Website invitation generation with `FOR UPDATE`, creates/restores membership, marks the invitation accepted, consumes the Auth continuation, and commits those SQL effects atomically.
